        ...
        <div class="row">
                ...
                <div class="post-slideshow">
                    <?php if(count($meta_values) > 0){
                        foreach($meta_values as $image){
                            $urls = wp_get_attachment_image_src($image,'thumbnail-770x514');
                            $img = '';
                            if(count($urls)>0)
                                $img = $urls[0];
                            ?>
                            <div class="item"><img alt="grant portfolio" src="<?php echo esc_url($img) ?>" /></div>
                        <?php }
                    }else { if(count($imgThumbs)>0) {?>
                        <div class="item"><img alt="grant portfolio" src="<?php echo esc_url($imgThumbs[0])?>" /></div>
                    <?php }
                    }
                    ?>
                </div>
            ...
            <div class="col-md-4 portfolio-content">
                <div class="portfolio-info">
                    <div class="title clear-top"><?php esc_html_e('Project Detail','zorka') ?></div>
